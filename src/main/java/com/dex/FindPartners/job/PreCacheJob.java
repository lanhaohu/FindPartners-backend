package com.dex.FindPartners.job;
...
public class PreCacheJob {
    ...
    public void doCacheRecommendUser() {
        RLock lock = redissonClient.getLock("FindPartners:precachejob:docache:lock");
        try {
            //只有一个线程可以获取到锁
            if (lock.tryLock(0, 30000L, TimeUnit.MILLISECONDS)){
                ...
                for (Long userId : mainUserList) {
                    ...
                    String redisKey = String.format("FindPartners:user:recommend:%s", userId);
